...
minContentWidth:   500      ,// pixels
minContentHeight:  450      ,// pixels
...
/*** <Navigation Options> ***/
navType:            'overlay'    ,// 'overlay'|'button'|'both'|'none'
...
init: function() {
	...
	if (!t.isChild) {
		...
		if (!t.graphicsPath) {
			...
			if ((match = /(?:url\()?["']?(.*)error.jpg["']?\)?$/i.exec(t.getStyle(node, 'background-image')))) {
