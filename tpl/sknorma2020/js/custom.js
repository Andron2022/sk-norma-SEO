$(function() {
    ...
    const sidebarHeight = $('.sidebar-scrolled-z').height()
    const sidebarScrTop = $('.sidebar-scrolled-z').offset().top
    const windowHeight = $(window).height()
    const footerHeight = $('.page-prefooter').height()+$('.page-footer').height()
    let currentscrTop = 0
    let currentSidebarScrTop

    if($(window).scrollTop() > sidebarScrTop) {
        $('.sidebar-scrolled-z').addClass('fixed').css('top', 10)
    }

    $(window).on(`scroll`, function () {
        if(sidebarHeight > $('.col-md-9').height()) {
            $('.sidebar-scrolled-z').removeClass('fixed')
        } else {
            if(sidebarHeight > $(window).height()) {
                if($(this).scrollTop() > currentscrTop) {
                    if($(this).scrollTop() >= sidebarScrTop+sidebarHeight-windowHeight) {
                        if(!$('.sidebar-scrolled-z').hasClass('sticky')) {
                            $('.sidebar-scrolled-z')
                                .removeClass('fixed')
                                .addClass('sticky')
                                .css('margin-top', $(this).scrollTop()-$('.page-wrapper-top').height()-10)
                                currentSidebarScrTop = $('.sidebar-scrolled-z').offset().top
                        }
                    }
                    if($(this).scrollTop() >= currentSidebarScrTop+(sidebarHeight-$(window).height())) {
                        $('.sidebar-scrolled-z').addClass('fixed').css('bottom', 0).css('top', 'inherit').css('margin-top', 'inherit')
                    }
                    if($(this).scrollTop() >= $(document).height() - (windowHeight+footerHeight)) {
                        $('.sidebar-scrolled-z')
                            .removeClass('fixed')
                            .css('margin-top', $('.page-wrapper-top').height()+$('.page-wrapper-middle').height()-(sidebarHeight+footerHeight+60))
                        currentSidebarScrTop = $('.sidebar-scrolled-z').offset().top
                    }
                } else {
                    if($(this).scrollTop() < $(document).height()-($(window).height()+footerHeight)) {
                        if($('.sidebar-scrolled-z').hasClass('sticky')) {
                            $('.sidebar-scrolled-z')
                                .removeClass('sticky')
                                .removeClass('fixed')
                                .css('margin-top', $(this).scrollTop()-(sidebarHeight-$(window).height())-$('.page-wrapper-top').height())
                            currentSidebarScrTop = $('.sidebar-scrolled-z').offset().top
                        }
                    }
                    if($(this).scrollTop() < currentSidebarScrTop) {
                        $('.sidebar-scrolled-z').addClass('fixed').css('bottom', 'inherit').css('top', 10).css('margin-top', 'inherit')
                    }
                    if($(this).scrollTop() < sidebarScrTop) {
                        $('.sidebar-scrolled-z').removeClass('fixed')
                    }
                }
                currentscrTop = $(this).scrollTop()
            } else {
                if($(this).scrollTop() > sidebarScrTop) {
                    $('.sidebar-scrolled-z').addClass('fixed').css('top', 10)
                } else {
