...
class Template_Lite {
	...
	var $cache			= true;	// whether or not to allow caching of files
	...
	var $cache_dir			= "compiled";	// where cache files are stored
	...
	var $reserved_template_varname = "simpla";
	...
	var $_version		= array('ver' => '2.3', 
                                'name' => 'Simpla!', 
                                'tpl' => 'v2.51 Simpla 6 May 2015  (c) 2005-2007 Mark Dickenson, (c) 2015 Simpla. All rights reserved. Released LGPL.');
	var $_version_date	= "2015-02-15 10:34:21";
	...
	function gratuito($str)
	{
		global $site, $db;
		$site_vars = isset($this->_vars['site_vars']) 
			? $this->_vars['site_vars'] : array();
			
		if(isset($site->vars)){
			$site_vars = $site->vars;
		}
		
		$var = "PHAgYWxpZ249ImNlbnRlciI+PHNtYWxsPjxhIGhyZWY9Imh0dHBzOi8vd3d3LjM2My5ydS9oZWxwL3NpbXBsYS8iIHRhcmdldD0iX2JsYW5rIj5Qb3dlcmVkIGJ5IFNpbXBsYSE8L2E+PC9zbWFsbD48L3A+CiAgICA8L2JvZHk+";
		
		$new_str = $str;
		$replace = "</body>";
		$replace_with = base64_decode($var);
		$pos = strripos($str, $replace);
		
		if(substr($this->_file, 0, 6) == 'emails' 
			|| substr($this->_file, 0, 5) == 'docs/'){
				return $str;		
		}

		if(!empty($site_vars['sys_licence']) && !empty($site_vars['id'])){
			// Подключаем коннект и класс валидации. 
			if(file_exists(MODULE.'/client/check_class.php')){
				require_once MODULE.'/client/check_class.php';  
				$lkey = !empty($site_vars['sys_licence']) 
					? $site_vars['sys_licence'] : '';
				
				//Если лицензионный ключ пустой - умираем и начинаем выводить форму
				if(!empty($lkey)){
					// Instantiate the class.  
					$protect=new protect();  
					
					// The license key string to validate.  
					// eg. $spbas->license_key='TEST-51b01e29e';  
					$protect->license_key = $lkey;
					
					// Enter the full URL to the api/ directory of the SPBAS install.  
					$protect->api_server='https://www.363.ru/module/validator/check.php';  
					
					$protect->secret_key='75156dd8dbd6df5a9f6246df2def7e5a';  
					$secret_key = !empty($site_vars['sys_licence_key']) 
						? $site_vars['sys_licence_key'] 
						: $protect->secret_key;    
			
					// Вызываем функцию валидации 
					$protect->site_id($site_vars['id'],$lkey,$secret_key);  
					$protect->validate(); 
					if(!isset($protect->payd_till)){
						$protect->payd_till = time();
					}
					
					
					if(isset($this->_vars['site_vars'])){
						$this->_vars['licence_payd_till'] = $protect->payd_till;
					}else{
						$site->vars['licence_payd_till'] = $protect->payd_till;
					}			
		...
		if ($pos !== false){
			//$new_str = substr_replace($str, $replace_with, $pos, strlen($replace));
			$new_str = preg_replace("!".$replace."!is", $replace_with, $str);
		}
		return $new_str;
	}
...
//		$this->_error_level = error_reporting(E_ALL);
	...
	function fetch_html($html, $cache_id = null, $display = false)
	{

		$output = $this->_fetch_compile_html($html);
	...
	function _fetch_compile_html($file_contents)
	{
	...
	function _run_modifier()
	{
		...
		else
		{
			if($php_function == "PHP")
			{
				if($modifier == 'count' && empty($arguments[0])){
					return $variable;
				}
