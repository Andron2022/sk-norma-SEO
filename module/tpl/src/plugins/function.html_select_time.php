...
function tpl_function_html_select_time($params, &$template_object)
{
    ...
    if ($display_hours)
	{
		...
		$sel = empty($params['time']) ? 0 : strftime($hour_fmt, $time);
    ...
    if ($display_minutes)
	{
		...
		$selected = empty($params['time']) 
			? 0 
			: intval(floor(strftime('%M', $time) / $minute_interval) * $minute_interval);
    ...
    if ($display_seconds)
	{
		...
		$selected = empty($params['time']) 
			? 0 
			: intval(floor(strftime('%S', $time) / $second_interval) * $second_interval);
